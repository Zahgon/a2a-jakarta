 ...
 */
/**
 * Registered with the embedded gRPC server by {@code @GrpcService}.
 *
 * <p>This is what the WildFly gRPC subsystem did implicitly: it scanned the deployment for
 * {@code BindableService} implementations and bound them to the server it managed. Nothing in
 * Spring does that on its own, so without this annotation the class compiles, the beans initialise,
 * and no gRPC endpoint is ever served — the transport is dead code that looks migrated.
 */
@GrpcService
public class SpringGrpcHandler_v0_3 extends GrpcHandler_v0_3 {
